...
export const transformAST: ASTTransformation = ({ j, root }) => {
  ...
  const modelCollection = root
    .find(j.ObjectProperty, node => (node.key as Identifier).name === 'model')
  ...
  const propPath = modelCollection
    .find(j.ObjectProperty, (node: ObjectProperty) => {
      // @ts-ignore
      return (node.key as Identifier).name === 'prop'
    })
    .get(0)
  ...
  const eventPath = modelCollection
    .find(j.ObjectProperty, (node: ObjectProperty) => {
      // @ts-ignore
      return (node.key as Identifier).name === 'event'
    })
    .get(0)
  ...
  const propsCollections = root
    .find(j.ObjectProperty, node => (node.key as Identifier).name === 'props')
    .filter(path => path.parent.parent.node.type === 'ExportDefaultDeclaration')
  if (!propsCollections.length) return

  // find the value which is in props
  const valueNodePath = propsCollections
    .find(j.ObjectProperty, node => (node.key as Identifier).name === propName)
  ...
  const methodsCollections = root
    .find(j.ObjectProperty, node => (node.key as Identifier).name === 'methods')
    .filter(
      nodePath =>
