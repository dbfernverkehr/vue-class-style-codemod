...
export const transformAST: ASTTransformation = ({ root, j }) => {
  ...
  const emitCalls = defaultExportBody.find(j.CallExpression, node => {
    return (
        (node.callee as MemberExpression).object?.type === 'ThisExpression' &&
        ((node.callee as MemberExpression).property as Identifier)?.name === '$emit'
    )
  })
  ...
  if (emitCalls.length) {
    ...
    const emitsProperty = defaultExportBody.find(j.ObjectProperty, node => {
      return (node.key as Identifier).name === 'emits' && node.value.type === 'ArrayExpression'
    })
